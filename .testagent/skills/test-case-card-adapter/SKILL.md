...
cd <root>\.testagent\skills\test-case-card\scripts; python -u card_generate.py `
  "<root>\.design_output\<design_task_id>\<IR>\ts_<NN>_test_case.json" `
  "<spec-file>" `
  "<cida-info>" `
  "<IR>_<ts-id>"
```
...
- 命令间隔必须使用 `;`；
- 不得使用 `&&`；
- 当前 TS 的初始化 key 必须是 `<IR>_<ts-id>`；
- 第 4 个参数必须传入 `<IR>_<ts-id>`；
...
- 调用 `test-case-card/scripts/card_generate.py` 时必须传入第 4 个参数 `<IR>_<ts-id>`；
