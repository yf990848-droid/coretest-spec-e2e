...
下面是一份完整文件示例，包含前半部分分析正文、DFX 规格和末尾固定表格。实际生成时，将示例文字替换为当前需求的分析结果，每个固定标题下都要有正文。

有多个 DFX TS 时，在同一个“DFX TS 测试规格”标题下，按真实平台 ID 分别写一段；没有 DFX 时省略这一部分。DFX 不放进“TS 清单”，清单每行是一个普通 TS，只允许 `scene/function/feature/constraint`。
...
下面以 constraint 类型为例，展示完整文件：前半部分写测试点设计，末尾放元信息和 TP 表。其他类型按上表使用适用标题，每个标题下都要有正文；增加测试点时，正文和 TP 表同步增加。
...
本规格验证链路故障后的重选次数和周期控制。本测试点覆盖次数的最大值和最小值；周期控制可按实际需求继续补充测试点。

### TP.01.01.01 重选次数边界控制

- 测试点描述：验证最大值和最小值对应的重选行为。
- 等级：Level1。
- 验证范围：分别配置允许的最大重选次数和最小重选次数，注入链路故障，检查实际重选次数及达到上限后的处理。
- 关联因子：无。
- 所属维度：基于业务内部实现。
...
下面是一份完整文件示例：前半部分按测试点展示用例详情，末尾放固定 TC 表。增加用例时，前面的说明和后面的表格同步增加。
...
- 用例编号：UPCF_RESELECT_01_01。
- 等级：Level1。
- 测试类型：功能性-功能正确性（TestType=1）。
- 自动化类型：非自动化（AutoType=0）。
- 测试环境类型：留空。
- 设计描述：验证链路故障后系统按次数边界约束执行并停止重选。
